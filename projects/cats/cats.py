...
def pick(paragraphs: list[str], select, k: int) -> str:
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    filter_paragraph = [paragraph for paragraph in paragraphs if select(paragraph)]
    if k >= len(filter_paragraph):
        return ""
    else:
        return filter_paragraph[k]
    # END PROBLEM 1
...
def about(keywords: list[str]):
    ...
    def search_if_in(paragraph: str):
...
def accuracy(typed: str, source: str) -> float:
    ...
    if typed_len == 0 and src_len == 0:
        return 100.0
    elif typed_len == 0 and src_len != 0:
        return 0.0
    elif typed_len != 0 and src_len == 0:
        return 0.0
    else:
        matched_times = 0
        useful_len = min(typed_len, src_len)
        for i in range(useful_len):
            if typed_words[i] == source_words[i]:
                matched_times += 1
        return matched_times / typed_len * 100
    # END PROBLEM 3
...
def memo_diff(diff_function):
    ...
    def memoized(typed, source, limit):
        # BEGIN PROBLEM EC
        "*** YOUR CODE HERE ***"
        key_pair = (typed, source)
        if key_pair not in cache:
            result = diff_function(typed, source, limit)
            cache[key_pair] = {'value': result, 'limit': limit}
            return result
        else:
            old_limit = cache[key_pair]['limit']
            if old_limit >= limit:
                return cache[key_pair]['value']
            else:
                new_result = diff_function(typed, source, limit)
                cache[key_pair] = {'value': new_result, 'limit': limit}
                return new_result
        # END PROBLEM EC
...
@memo
def autocorrect(
    typed_word: str, word_list: list[str], diff_function, limit: int
) -> str:
    ...
    # BEGIN PROBLEM 5
    "*** YOUR CODE HERE ***"
    autocorrected_word = typed_word
    diff = float("inf")
    for word in word_list:
        if word == typed_word:
            return typed_word
        curr_diff = diff_function(typed_word, word, limit)
        if curr_diff <= limit and diff > curr_diff:
            autocorrected_word = word
            diff = curr_diff
    return autocorrected_word
    # END PROBLEM 5
...
def time_per_word(words: list[str], timestamps_per_player: list[list[int]]) -> dict:
    ...
    # BEGIN PROBLEM 9
    times = [
        [tpp[i][j] - tpp[i][j - 1] for j in range(1, len(tpp[i]))]
...
def fastest_words(words_and_times: dict) -> list[list[str]]:
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    each_fast_words = [[] for _ in player_indices]
    for i in word_indices:
        fast_time = min([get_time(times, j, i) for j in player_indices])
        fast_player = [j for j in player_indices if times[j][i] == fast_time][0]
        each_fast_words[fast_player] += [words[i]]
    return each_fast_words
    # END PROBLEM 10
