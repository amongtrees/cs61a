...
class Keyboard:
    ...
    def __init__(self):
        self.typed = []
        self.keys = {c: Button(c, self.append_typed) for c in LOWERCASE_LETTERS}  # Try a dictionary comprehension!
    
    def append_typed(self, c):
            self.typed.append(c)
...
class WinkingBear(Bear):
    ...
    def __init__(self):
        "*** YOUR CODE HERE ***"
        super().__init__()
        self.curr_closed = False

    def next_eye(self):
        "*** YOUR CODE HERE ***"
        self.curr_closed ^= True
        return Eye(self.curr_closed)
...
class Counter:
    ...
    def inc(self):
        self.count += 1
        if self.parent:
            self.parent.inc()       # Here we must do it in a recursive way.
...
class MissDict:
    ...
    def __str__(self):
        return f'{self.misses.count}/{self.misses.parent.count} of the misses'
