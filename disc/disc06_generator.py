...
# The (lambda t: ...) part is a function and (gen_fib()) is then a call on this func. In the func, an iterator is taken as an argument (specifically here, the gen_fib() func is the iterator, also a generator). This func returns a list which creates every item of itself by calling next() on the iterator.
